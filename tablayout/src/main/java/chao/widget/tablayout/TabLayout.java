...
import static android.support.design.R.layout.design_layout_tab_icon;
import static android.support.design.R.layout.design_layout_tab_text;
import static android.support.design.R.style.TextAppearance_Design_Tab;
import static android.support.design.R.style.Widget_Design_TabLayout;
import static android.support.design.R.styleable.TabLayout_tabBackground;
import static android.support.design.R.styleable.TabLayout_tabContentStart;
import static android.support.design.R.styleable.TabLayout_tabGravity;
import static android.support.design.R.styleable.TabLayout_tabIndicatorColor;
import static android.support.design.R.styleable.TabLayout_tabIndicatorHeight;
import static android.support.design.R.styleable.TabLayout_tabMaxWidth;
import static android.support.design.R.styleable.TabLayout_tabMinWidth;
import static android.support.design.R.styleable.TabLayout_tabMode;
import static android.support.design.R.styleable.TabLayout_tabPadding;
import static android.support.design.R.styleable.TabLayout_tabPaddingBottom;
import static android.support.design.R.styleable.TabLayout_tabPaddingEnd;
import static android.support.design.R.styleable.TabLayout_tabPaddingStart;
import static android.support.design.R.styleable.TabLayout_tabPaddingTop;
import static android.support.design.R.styleable.TabLayout_tabSelectedTextColor;
import static android.support.design.R.styleable.TabLayout_tabTextAppearance;
import static android.support.design.R.styleable.TabLayout_tabTextColor;
import static android.support.design.R.styleable.TextAppearance_android_textColor;
import static android.support.design.R.styleable.TextAppearance_android_textSize;
...
/**
 *
 * 由于android.support.design.widget.TabLayout很多TabView等元素都没有开放出来，
 *
 * 所以这里将TabLayout拷贝过来，用来适应项目中的一些特殊需求和规格。
 *
 * by chao.qin 2017/03/13
 *
 */
public class TabLayout extends HorizontalScrollView {

    private static final int DEFAULT_HEIGHT_WITH_TEXT_ICON = 72; // dps
    private static final int DEFAULT_GAP_TEXT_ICON = 6; // dps
    private static final int INVALID_WIDTH = -1;
    private static final int DEFAULT_HEIGHT = 48; // dps
    private static final int TAB_MIN_WIDTH_MARGIN = 0; //dps
    ...
    public static final int MODE_FIXED_COMMON = 2;

    /**
     * 
     */
    @IntDef(value = {MODE_SCROLLABLE, MODE_FIXED, MODE_FIXED_COMMON})
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TabLayout,
                defStyleAttr, Widget_Design_TabLayout);

        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(TabLayout_tabIndicatorHeight, 0));
        ...
        mTabPaddingStart = a.getDimensionPixelSize(TabLayout_tabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(TabLayout_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(TabLayout_tabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(TabLayout_tabPaddingBottom,
                mTabPaddingBottom);

        mTabTextAppearance = a.getResourceId(TabLayout_tabTextAppearance,
        ...
        mRequestedTabMinWidth = a.getDimensionPixelSize(TabLayout_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(TabLayout_tabMaxWidth,
                INVALID_WIDTH);
        ...
        mTabSelectedTextSize = a.getDimensionPixelSize(chao.widget.tablayout.R.styleable.TabLayout_tabSelectedTextSize, (int) mTabTextSize);
...
//        mScrollableTabMinWidth = res.getDimensionPixelSize(R.dimen.design_tab_scrollable_min_width);
    ...
    private void updateTabViewLayoutParams(LinearLayout.LayoutParams lp) {
        if ((mMode == MODE_FIXED || mMode == MODE_FIXED_COMMON) && mTabGravity == GRAVITY_FILL) {
            lp.width = 0;
            lp.weight = 1;
        } else {
...
//        }
    ...
    class TabView extends LinearLayout implements OnLongClickListener {
        ...
        public void onMeasure(final int origWidthMeasureSpec, final int origHeightMeasureSpec) {
            ...
            //MODE_FIXED_COMMON 在这种情况下会显示异常chao.qin
            if (mMode != MODE_FIXED_COMMON && maxWidth > 0 && (specWidthMode == MeasureSpec.UNSPECIFIED
...
//
//                    if ((mMode == MODE_FIXED || mMode == MODE_FIXED_COMMON) && textSize > curTextSize && curLineCount == 1) {
...
//            }
            ...
            if (mCustomView == null) {
                ...
                if (mIconView == null) {
                    ImageView iconView = (ImageView) LayoutInflater.from(getContext())
                            .inflate(design_layout_tab_icon, this, false);
                    addView(iconView, 0);
                    mIconView = iconView;
                }
                if (mTextView == null) {
                    TextView textView = (TextView) LayoutInflater.from(getContext())
                            .inflate(design_layout_tab_text, this, false);
                    addView(textView);
                    mTextView = textView;
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
            ...
            if ((mMode == MODE_FIXED || mMode == MODE_FIXED_COMMON) && mTabGravity == GRAVITY_CENTER) {
        ...
        private void updateIndicatorPosition() {
            final TabView selectedTabView = (TabView) getChildAt(mSelectedPosition);
            View selectedTitle = selectedTabView.mTextView;
            if (selectedTabView.mCustomTextView != null) {
                selectedTitle = selectedTabView.mCustomTextView;
            }
            if (selectedTitle == null){
                selectedTitle = selectedTabView;
            }
            int left, right;

            if (selectedTitle != null && selectedTitle.getWidth() > 0) {

                left = selectedTabView.getLeft() + selectedTitle.getLeft();
                right = selectedTabView.getLeft() + selectedTitle.getRight();

                if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                    // Draw the selection partway between the tabs
                    TabView  nextTabView = (TabView) getChildAt(mSelectedPosition + 1);
                    TextView nextTitle = nextTabView.mTextView;
                    if (nextTabView.mCustomTextView != null) {
                        nextTitle = nextTabView.mCustomTextView;
                    }
                    left = (int) (mSelectionOffset * (nextTabView.getLeft() + nextTitle.getLeft()) +
                            (1.0f - mSelectionOffset) * left);
                    right = (int) (mSelectionOffset * (nextTabView.getLeft() + nextTitle.getRight()) +
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            final TabView tabView = (TabView) getChildAt(position);
            View targetView = tabView.mTextView;
            if (tabView.mCustomTextView != null) {
                targetView = tabView.mCustomTextView;
            }
            if (targetView == null) {
            ...
            final int targetLeft = tabView.getLeft() + targetView.getLeft();
            final int targetRight = tabView.getLeft() + targetView.getRight();
            final int startLeft;
